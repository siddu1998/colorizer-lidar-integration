...
for i,row in df1.iterrows():
    ...
    if row['Retro']<0.4:
        x3_very_poor.append(row['X'])
        y3_very_poor.append(row['Y'])
        dz_very_poor.append(row['Retro'])
    if 0.41<row['Retro']<0.5:
        x3_average_point.append(row['X'])
        y3_average_point.append(row['Y'])
        dz_average_points.append(row['Retro'])
    if 0.51<row['Retro']<0.6:
        x3_above_average_point.append(row['X'])
        y3_above_average_point.append(row['Y'])
        dz_above_average_point.append(row['Retro'])
    if row['Retro']>0.61:
        x3_great_point.append(row['X'])
        y3_great_point.append(row['Y'])
...
def updated_graph_points(df,value_title):
    ...
    dx=0.03
    dy=0.03
    ax_pop.bar3d(df['X'],df['Y'], 0, dx, dy, df['Retro'],color='b')
...
class PageTwo(tk.Frame):
    ...
    def __init__(self,parent,controller):
        ...
        self.subplot_kw = dict(autoscale_on=True)
    ...
    def enter_click(self):
        ...
        for point in red_points_selected[0]:            
            row_red=df1.loc[(df1['X'] == point[0]) & (df1['Y'] == point[1])]
        ...
        for point in white_points_selected[0]:
            row_white=(df1.loc[(df1['X'] == point[0]) & (df1['Y'] == point[1])])
        ...
        for point in blue_points_selected[0]:     
            row_blue=df1.loc[(df1['X'] == point[0]) & (df1['Y'] == point[1])]
        ...
        for point in green_points_selected[0]:
            row_green=df1.loc[(df1['X'] == point[0]) & (df1['Y'] == point[1])]
        ...
        for point in yellow_points_selected[0]:
            row_yellow=df1.loc[(df1['X'] == point[0]) & (df1['Y'] == point[1])]
    ...
    def update_histograms_based_on_selected(self):
        ...
        df_red_retro_selected = df_red_retro_selected[['SignId','X','Y','Retro']]
        df_white_retro_selected=df_white_retro_selected[['SignId','X','Y','Retro']]
        df_blue_retro_selected=df_blue_retro_selected[['SignId','X','Y','Retro']]
        df_green_retro_selected=df_green_retro_selected[['SignId','X','Y','Retro']]
        df_yellow_retro_selected=df_yellow_retro_selected[['SignId','X','Y','Retro']]
